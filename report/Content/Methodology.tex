...
salidas del modelo fueron corregidas mediante una red neuronal artificial, con
el fin de ajustar los caudales simulados a los datos observados disponibles en
la cuenca.
...
\node[proc] (nn) at (0, -5.9) {Corrección por Redes Neuronales}; \draw[arr]
(routing) -- (nn);
% Fila 6: salida
\node[resbox] (qnat) at (0, -7.1) {Caudales naturalizados mensuales\\
     QN-908 / QN-909 / QN-910 / QN-901,911 / QN-903,905}; \draw[arr] (nn) --
(qnat);
\end{tikzpicture}
...
las variables necesarias para forzar el modelo hidrológico \textbf{VIC},
...
ajuste por cuantiles (\textbf{quantile mapping}) con el fin de mejorar la
...
\toprule
\textbf{Indicador} & Altos Machay & Chalhuacocha & Hacienda Huanca & Huachon &
...
\begin{figure}[H]
    ...
    \caption{Ubicación de estaciones virtuales para las zonas alta (EstAltaWS), media (EstMediaWS) y baja(EstBajaWS) de la cuenca (Fuente: Elaboracion Propia)}
    \label{fig:4}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Hidrografía regional: ubicación del reservorio Yuncán (QN-906) y de las estaciones hidrológicas asociadas QN-908 (Uchuhuerta), QN-909 (Huallamayo). (Fuente: Elaboracion Propia)}
    \captionsetup{labelfont=rm,skip=2pt,textfont=rm,font=small}
    \label{fig:4f}
\end{figure}
...
\begin{tabular}{l c c c c}
\toprule
\textbf{Parámetro} & \textbf{Qn-905 (Yuncan)} & \textbf{QN-908 (Uchuhuerta)} &
...
Km \\
Pendiente Media del Rio (lc)                    & 1\%    & 5\%    & 6\%      &
m/m \\
...
representativos de estiaje y avenida, aplicando la ecuación de Manning para el
Túnel 1 (pelo libre) y la relación $v = Q/A$ para los túneles a presión y el
conducto forzado.

La Tabla~\ref{tab:tt_uchu} presenta los resultados para la \textbf{Ruta A}
(Bocatoma Uchuhuerta~→ Casa de Máquinas, $\approx$20\,487~m).
...
\toprule
\textbf{Q} & \textbf{Época} & \textbf{T1 (h)} & \textbf{T2 (h)} & \textbf{T4
(h)} & \textbf{CF (h)} & \textbf{Total (h)} \\
...
\small\textit{T1: Túnel 1 (pelo libre); T2: Túnel 2; T4: Túnel 4; CF: Conducto
Forzado.}
\end{minipage}
\end{table}

La Tabla~\ref{tab:tt_hual} presenta los resultados para la \textbf{Ruta B}
...
Los tiempos resultantes oscilan entre \textbf{3.0 y 6.9~h} para la Bocatoma
Uchuhuerta ($Q = 5$--$20$~m\textsuperscript{3}/s, limitado por la capacidad del
Túnel~1), y entre \textbf{0.64 y 6.4~h} para el Embalse Huallamayo ($Q =
30$--$3$~m\textsuperscript{3}/s según la serie QN-909).
...
se empleó el modelo hidrológico distribuido \textbf{VIC (Variable Infiltration
...
una red neuronal artificial (RNA). Este enfoque permite reducir sesgos
sistemáticos y inconsistencias residuales entre los caudales simulados y los
observados, sin modificar la estructura física del modelo.

La red neuronal fue entrenada utilizando las series temporales de caudales
simulados y observados en las estaciones con registro disponible, para el
período de calibración 2002–2020. La validación se realizó sobre el período
2021–2025. Las variables de entrada a la red incluyeron los caudales diarios
simulados por VIC junto con variables meteorológicas auxiliares (precipitación
mensual acumulada y temperatura media). La arquitectura empleada corresponde a
una red de múltiples capas densas (\textit{Multi-Layer Perceptron}, MLP),
entrenada mediante retropropagación con regularización para prevenir el
sobreajuste.

Los resultados muestran que la corrección mediante redes neuronales mejoró de
manera consistente los indicadores estadísticos en todos los puntos de control
evaluados, especialmente el NSE y el KGE, confirmando la capacidad del esquema
híbrido VIC + RNA para representar la dinámica hidrológica observada en la
cuenca.

\newpage
\section{DISCUSIONES Y CONCLUSIONES}

El presente estudio tiene como finalidad actualizar y presentar las series de
caudales naturalizados del sistema hídrico de la cuenca del río Paucartambo,
utilizadas por la Central Hidroeléctrica Yuncán de ENGIE Energía Perú S.A., en
cumplimiento de la Resolución OSINERGMIN N° 196-2016-OS/CD y del Procedimiento
Técnico N°41. El periodo de análisis comprende desde 1965 hasta el año 2025, con
énfasis en la simulación hidrológica para el intervalo 1998–2025.

El cálculo de los caudales naturales se ha basado en una metodología de
hidrología física distribuida, mediante el uso del modelo VIC (Variable
Infiltration Capacity), alimentado con datos meteorológicos diarios y variables
geofísicas de alta resolución. Para este estudio, se utilizó precipitación
proveniente del producto satelital CHIRPS y variables meteorológicas del
reanálisis ERA5-Land, corrigiendo la primera serie por técnicas de
\textit{quantile mapping}. El área de estudio fue modelada hidrológicamente en
una malla distribuida con resolución de ~5 km, y los caudales simulados fueron
posteriormente corregidos mediante una red neuronal artificial para ajustar los
resultados a los datos observados disponibles.

Los caudales promedio mensuales fueron simulados para el periodo 1998–2025,
utilizando un periodo de estabilización (\textit{spin-up}) de cuatro años, lo
que permitió obtener resultados robustos en los puntos de control hidrológico
definidos: QN-908 (Uchuhuerta), QN-909 (Huallamayo), así
como en puntos de control adicionales como QN-901 + QN-902 + QN-911 y QN-903 +
QN-904 + QN-905.

Los resultados obtenidos muestran un buen nivel de concordancia entre los
caudales simulados y los observados, validado mediante indicadores estadísticos
como el NSE (coeficiente de eficiencia de Nash-Sutcliffe), RSR, PBIAS, índice de
Willmott (d), coeficiente de correlación (r), coeficiente de determinación
(R$^2$) y KGE, presentando valores calificados como ``Muy Buenos'' o ``Buenos''
en todos los puntos evaluados (Tablas \ref{tab:swat_qn908} a
\ref{tab:swat_qn903_qn905}).

Se concluye que:

\begin{enumerate}
    \item El contenido y estructura del estudio cumplen con los lineamientos
    establecidos en la Resolución OSINERGMIN N° 196-2016-OS/CD y el
    Procedimiento Técnico N°41, considerando adecuadamente el periodo 1965–2025.
    
    \item Se ha actualizado la serie de caudales naturalizados y de
    precipitaciones mensuales utilizando metodologías modernas basadas en
    información satelital de alta resolución y reanálisis climático.
    
    \item El uso de datos satelitales CHIRPS y ERA5-Land ha permitido mejorar la
    representación espacio-temporal de las condiciones climáticas de la cuenca.
    La validación frente a estaciones meteorológicas locales demuestra su
    confiabilidad.
    
    \item La implementación del modelo VIC, complementada con la corrección por
    redes neuronales, permitió reproducir con buen desempeño la dinámica
    hidrológica mensual de la cuenca, representando una base sólida para futuras
    simulaciones y planificación energética. Se recomienda continuar con el
    proceso de mejora de la red hidrometeorológica, refinamiento de parámetros
    del modelo y actualización periódica de la base de datos.
